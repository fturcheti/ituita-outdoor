...
ituitaBlobTracker::ituitaBlobTracker(){
	IDCounter = 0;
	numEnter = 0;
	numLeave = 0;
	nSize = 0; 
...
void ituitaBlobTracker::update( ofxCvGrayscaleImage& input, int _threshold, int _minArea,int _maxArea , int _nConsidered , double _hullPress , bool _bFindHoles , bool _bUseApproximation ){
...
void ituitaBlobTracker::track(ofxContourFinder* newBlobs){	
	numEnter = 0;
	numLeave = 0;
 ...
 **************************************************************************/
int ituitaBlobTracker::trackKnn(ofxContourFinder *newBlobs, ofxBlob *track, int k, double thresh){    
...
void ituitaBlobTracker::draw( float _x, float _y, float _width, float _height ) {
    ...
    for( int i=0; i<(int)trackedBlobs.size(); i++ ) {
        ...
        ofSetColor(255, 0, 0);
        ofDrawBitmapString(ofToString(trackedBlobs[i].id), trackedBlobs[i].centroid.x*width, trackedBlobs[i].centroid.y*height );
